---
title: "Beginner's Guide to Text Mining & Sentiment Analysis"
...
* cleanCorpus() - The first function performs cleaning and preprocessing steps to a corpus. A Corpus is a set of texts. We are using this function for preprocessing out texts.
...
* frequentTerms() - This function constructs the term-document matrix from clean corpus, it describes the frequency of terms that occur in a collection of documents. This matrix has terms in the first column and documents (sentences) across the top as individual column names.
...
(3) frequentBigrams() - It extracts token containing two words (bigrams). we will use this function two make plots for bigrams.
...
Let's get the total number of dialogue in Episode IV.

```{r}
# Total number of dialogues.
ep4 %>%
  summarise(dialogue_number = n())
```
...
We can see there are total 60 character in Episode IV - A New Hope. We will now construct a frequency plot for showing top 15 character in Episode IV: A New Hope.
...
frequentBigrams(ep4$dialogue) %>%
  ...
  coord_flip() +
  theme_gdocs() +
  labs(x="Word Frequency", y="Bigrams",title="Star Wars Episode IV")
```
...
There are two variables -
  ...
  * character - Number of character in episode VI.
...
Wow, there are 53 characters in Episode VI. We will repeat the same procedure with Episode VI for generating the plot to check the top characters and frequent terms & Bigrams.
